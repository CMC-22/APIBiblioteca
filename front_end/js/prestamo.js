let url = "http://localhost:8080/api/v1/prestamo/";
...
function eliminarPrestamo(id_prestamo) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: 'http://localhost:8080/api/v1/prestamo/eliminarPermanente/' + id_prestamo,
          type: "DELETE",
          success: function (result) {
  ...
  function obtenerUnPrestamo(id) {
    $.ajax({
      ...
      if (document.getElementById("editUsuario")) {
        document.getElementById("editUsuario").value = prestamo.usuario;
      }
      if (document.getElementById("editLibro")) {
        document.getElementById("editLibro").value = prestamo.libro;
      }
      ...
      error: function (error) {
        ...
        Swal.fire(
          'Error!',
          'Hubo un problema al intentar obtener los detalles del prestamo.',
          'error'
        );
      }
    });
